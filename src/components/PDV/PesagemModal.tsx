...
export function PesagemModal({ produto, onConfirmar, onFechar, useDirectScale = false }: { 
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-2 md:p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-2 md:mx-0 overflow-hidden">
        {/* Header com gradiente */}
        <div className="bg-gradient-to-r from-blue-600 to-green-500 p-4 md:p-6 text-white relative overflow-hidden">
          <div className="absolute inset-0 bg-black/10"></div>
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-3">
                <div className="bg-white/20 rounded-full p-2 backdrop-blur-sm">
                  <Scale size={20} md:size={24} className="text-white" />
                </div>
                <h2 className="text-lg md:text-xl font-bold">Pesagem de Produto</h2>
              </div>
              {onFechar && (
                <button
            ...
            <p className="text-white/90 truncate text-sm md:text-base">{produto.name || produto.nome}</p>
          </div>
        </div>
        ...
        <div className="p-4 md:p-6 space-y-4 md:space-y-6">
          ...
          <div className="text-center">
            <button
              onClick={usarBalanca}
              disabled={loading}
              className={`w-full py-3 md:py-4 px-4 md:px-6 rounded-xl font-semibold text-base md:text-lg transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl ${
                loading 
              ...
              <div className="flex items-center justify-center gap-3">
                {loading ? (
                  ...
                  <>
                    ...
                    <Zap size={16} md:size={20} className="animate-pulse" />
                  </>
                )}
              </div>
            </button>
            ...
            <div className={`rounded-xl p-4 border-2 transition-all duration-300 ${
              ...
              <div className="flex items-center gap-3">
                <div className={`rounded-full p-2 ${
                  ...
                  {isDuplicate ? (
                    <AlertCircle size={16} md:size={20} className="text-yellow-600" />
                  ) : (
                    <Check size={16} md:size={20} className="text-green-600" />
                  )}
                </div>
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-3 md:px-4 bg-white text-gray-500 font-medium text-xs md:text-sm">ou digite o peso</span>
              </div>
            </div>
            ...
            <div>
              <input
                ...
                className="w-full p-3 md:p-4 text-base md:text-lg border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
              />
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 md:gap-3">
              {[100, 200, 300, 500, 750, 1000].map((g) => (
                <button
                  ...
                  className="py-2 px-2 md:px-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg font-medium transition-colors text-xs md:text-sm"
                >
                  {g}g
                </button>
              ))}
